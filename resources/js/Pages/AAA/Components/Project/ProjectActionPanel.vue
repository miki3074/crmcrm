...
<template>
    <Transition name="slide">
        <div v-if="show" class="fixed inset-0 z-[100] flex justify-end">
            ...
            <!-- Panel -->
            <div class="relative w-full max-w-md bg-white dark:bg-slate-900 h-full shadow-xl flex flex-col border-l border-slate-200 dark:border-slate-800">
                ...
                <div class="flex border-b border-slate-100 dark:border-slate-800">
                    <button @click="activeSection = 'settings'; initForm()"
                            :class="['flex-1 py-3 text-sm font-bold transition-all', activeSection === 'settings' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-400']">
                        Настройки
                    </button>
                    <button @click="activeSection = 'team'"
                            :class="['flex-1 py-3 text-sm font-bold transition-all', activeSection === 'team' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-400']">
                        Команда
                    </button>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar">
                    <!-- Section: Settings -->
                    <div v-if="activeSection === 'settings'" class="space-y-3 animate-in slide-in-from-right-4 duration-300">
                        <div class="space-y-2">
                        ...
                        <div class="pt-10 border-t border-slate-100 dark:border-slate-800">
                            <button @click="deleteProject" class="w-full py-3 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-2xl font-bold transition-all border border-rose-100">
                                Удалить проект
                            </button>
                        </div>
                    </div>
                    ...
                    <div v-if="activeSection === 'team'" class="space-y-3 animate-in slide-in-from-right-4 duration-300">
...
<style scoped>
.panel-input {
    @apply w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-800 dark:text-slate-200;
}
