...
<template>
    <div class="bg-white dark:bg-zinc-800 rounded-2xl p-4 shadow-sm border border-zinc-100 dark:border-zinc-700 h-auto flex flex-col">
        ...
        <div class="flex justify-between items-center mb-3">
            <h3 class="font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                ...
                Подзадачи <span class="text-xs text-zinc-400 font-normal bg-zinc-100 dark:bg-zinc-700 px-2 py-0.5 rounded-full">{{ subtasks?.length || 0 }}</span>
            </h3>
            <button
                v-if="canCreate"
                @click="$emit('create')"
        ...
        <!-- Пустое состояние -->
        <div v-if="!subtasks?.length" class="flex-1 flex flex-col items-center justify-center py-8 text-zinc-400 border-2 border-dashed border-zinc-100 dark:border-zinc-700 rounded-xl">
        ...
        <div v-else>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 mb-3">
                <div
                    ...
                    class="group relative bg-white dark:bg-zinc-700 p-4 rounded-xl border border-zinc-200 dark:border-zinc-600 hover:border-cyan-300 dark:hover:border-cyan-500 hover:shadow-md transition-all cursor-pointer flex flex-col justify-between"
                >
                    <!-- Верх: Заголовок -->
                    <div class="flex justify-between items-start mb-3">
                        <h4 class="text-sm font-semibold text-zinc-800 dark:text-zinc-200 line-clamp-2 group-hover:text-cyan-600 transition-colors pr-2 leading-5">
                            {{ s.title }}
                        </h4>
                        <!-- Стрелочка -->
                        <div class="opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                            <svg class="w-4 h-4 text-cyan-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                        </div>
                    </div>
                    ...
                    <!-- Низ: Аватары и Дата -->
                    <div class="flex items-center justify-between pt-2 border-t border-zinc-50 dark:border-zinc-700 mt-auto">
                        <!-- Аватары -->
                        <div class="flex -space-x-2 overflow-hidden pl-1">
                            <template v-if="s.executors?.length">
                                <div v-for="exe in s.executors.slice(0, 2)" :key="exe.id"
                                     class="inline-block h-6 w-6 rounded-full ring-2 ring-white dark:ring-zinc-800 bg-zinc-200 flex items-center justify-center text-[9px] font-bold text-zinc-600"
                                     :title="exe.name">
                                    {{ getInitials(exe.name) }}
                                </div>
                                <div v-if="s.executors.length > 2" class="inline-block h-6 w-6 rounded-full ring-2 ring-white dark:ring-zinc-800 bg-zinc-100 flex items-center justify-center text-[9px] font-bold text-zinc-500">
                                    +{{ s.executors.length - 2 }}
                                </div>
                            </template>
                            <span v-else class="text-[10px] text-zinc-400">Нет исп.</span>
                        </div>
            ...
            <div v-if="hasMore" class="flex justify-center pt-2 border-t border-zinc-100 dark:border-zinc-700">
                <button
                    @click="showAll = !showAll"
                    class="text-xs font-medium text-zinc-500 hover:text-cyan-600 flex items-center gap-1 transition-colors px-4 py-2 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-700"
                >
