...
<!--                        <span class="badge">{{ form.articles.length }} {{ form.articles.length === 1 ? 'статья' : 'статей' }}</span>-->
                    ...
                    <div class="header-right">
                        ...
                        <div class="save-indicator" :class="{ saving: isSaving, saved: isSaved }">
                            <span v-if="isSaving" class="save-text">
                            ...
                            <span v-else-if="isSaved" class="save-text">
                            ...
                            <button v-if="hasUnsavedChanges" @click="forceSave" class="save-now-btn" title="Сохранить сейчас">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        <!-- Кнопка сброса -->
                        <button @click="resetToDefault" class="reset-btn" title="Сбросить к шаблону">
                ...
                <div class="section-block">
                    ...
                    <div class="form-group">
                        ...
                        <textarea v-model="form.initiator" rows="3" class="form-textarea" @input="markUnsaved"></textarea>
                    </div>

                    <div class="grid-2">
                        <div class="form-group">
                            <label>Метка документа</label>
                            <input v-model="form.projectMark" placeholder="Проект" @input="markUnsaved" />
                        </div>
                        <div class="form-group">
                            <label>Вид акта</label>
                            <input v-model="form.docType" class="input-uppercase" placeholder="ФЕДЕРАЛЬНЫЙ ЗАКОН" @input="markUnsaved" />
                        </div>
                    </div>
                    ...
                    <div class="grid-2">
                        <div class="form-group">
                            ...
                            <input v-model="form.region" placeholder="Республики Татарстан" @input="markUnsaved" />
                        </div>
                        <div class="form-group">
                            <label>Должность подписанта</label>
                            <input v-model="form.signerTitle" placeholder="Президент" @input="markUnsaved" />
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Заголовок закона</label>
                        <textarea v-model="form.title" rows="3" class="form-textarea" @input="markUnsaved"></textarea>
                    </div>
                </div>
                ...
                <div class="section-block">
                    ...
                    <div class="form-group">
                        ...
                        <input v-model="form.signerName" placeholder="В.В. Путин" @input="markUnsaved" />
                    </div>
                </div>
            ...
            <div class="preview-panel">
                <div class="paper-container" ref="paperContainer">
                    <div class="paper-sheet" :style="paperStyle">
                        ...
                        <!-- Счетчик страниц -->
                        <div class="page-counter" v-if="totalPages > 1">
                            {{ currentPage }}/{{ totalPages }}
                        </div>
                    </div>

                    <!-- Индикаторы прокрутки -->
                    <div v-if="showScrollIndicators" class="scroll-indicator top" :class="{ visible: canScrollUp }">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M18 15l-6-6-6 6"/>
                        </svg>
                    </div>
                    <div v-if="showScrollIndicators" class="scroll-indicator bottom" :class="{ visible: canScrollDown }">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M6 9l6 6 6-6"/>
                        </svg>
                    </div>
...
const STORAGE_KEY = 'law_draft_data';
...
// Инициализируем форму с сохраненными данными или данными по умолчанию
const savedData = loadSavedData();
const form = reactive(savedData || { ...defaultForm, articles: [...defaultForm.articles] });

// --- СОСТОЯНИЯ СОХРАНЕНИЯ ---
const isSaving = ref(false);
const isSaved = ref(true);
const hasUnsavedChanges = ref(false);
const lastSavedData = ref(JSON.stringify(form));
...
// Дебаунс для автоматического сохранения
...
const resetToDefault = () => {
    if (confirm('Сбросить все изменения к шаблону? Несохраненные данные будут потеряны.')) {
        Object.assign(form, {
            ...defaultForm,
            articles: defaultForm.articles.map(a => ({ ...a }))
        });
        saveToStorage();
    }
};

// --- ВОТЧЕРЫ ДЛЯ АВТОСОХРАНЕНИЯ ---
watch(() => form.initiator, () => {
    markUnsaved();
    debouncedSave();
});

watch(() => form.projectMark, () => {
    markUnsaved();
    debouncedSave();
});
...
watch(() => form.signerTitle, () => {
    markUnsaved();
    debouncedSave();
});

watch(() => form.signerName, () => {
    markUnsaved();
    debouncedSave();
});
...
onMounted(() => {
    ...
    if (paperContainer.value) {
        paperContainer.value.addEventListener('scroll', handleScroll);
        handleScroll();
        showScrollIndicators.value = totalPages.value > 1;
    }
});
...
onUnmounted(() => {
    ...
    if (paperContainer.value) {
        paperContainer.value.removeEventListener('scroll', handleScroll);
    }
});

// Refs для работы с прокруткой
const paperContainer = ref(null);

// Состояния для прокрутки
...
// Вычисляем высоту контента для динамического расширения
const paperStyle = computed(() => {
    const baseHeight = 1123;

    const contentLength =
        form.initiator.length +
        form.title.length +
        form.articles.reduce((acc, article) => acc + article.title.length + article.content.length, 0);

    const estimatedPages = Math.max(1, Math.ceil(contentLength / 3000));
    totalPages.value = estimatedPages;

    const height = baseHeight * estimatedPages;
...
// Следим за прокруткой контейнера
const handleScroll = () => {
    if (!paperContainer.value) return;

    const { scrollTop, scrollHeight, clientHeight } = paperContainer.value;

    canScrollUp.value = scrollTop > 20;
    canScrollDown.value = scrollTop < scrollHeight - clientHeight - 20;

    const pageHeight = 1123;
    const currentPageCalc = Math.floor(scrollTop / pageHeight) + 1;
    currentPage.value = Math.min(currentPageCalc, totalPages.value);
};

// Обновляем индикаторы при изменении контента
watch(() => form.articles, async () => {
    await nextTick();
    handleScroll();
    showScrollIndicators.value = totalPages.value > 1;
}, { deep: true });

watch(() => form.initiator, async () => {
    await nextTick();
    handleScroll();
});

watch(() => form.title, async () => {
    await nextTick();
    handleScroll();
});
...
/* Preview Panel - обновленный для поддержки прокрутки */
.preview-panel {
    flex: 1;
...
/* Бумажный лист с динамической высотой */
.paper-sheet {
    position: relative;
    background: white;
    width: 21cm;
...
/* Scrollbar Styles для редактора */
