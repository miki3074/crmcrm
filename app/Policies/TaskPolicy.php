<?php

namespace App\Policies;

use Illuminate\Auth\Access\Response;
use App\Models\Task;
use App\Models\User;

class TaskPolicy
{
    /**
     * Determine whether the user can view any models.
     */
    public function viewAny(User $user): bool
    {
        //
    }

    /**
     * Determine whether the user can view the model.
     */
    public function view(User $user, Task $task): bool
    {
        return
            $user->id === $task->creator_id ||               // создатель
            $user->id === $task->executor_id ||              // исполнитель
            $user->id === $task->responsible_id ||           // ответственный
            $user->id === $task->project->manager_id ||      // менеджер проекта
            $user->id === $task->project->company->user_id || // владелец компании
            $task->subtasks()->where('executor_id', $user->id)->exists();
    
    }

    /**
     * Determine whether the user can create models.
     */
    public function create(User $user): bool
    {
        //
    }

    public function createSubtask(User $user, Task $task): bool
{
    return $user->id === $task->responsible_id || $user->id === $task->project->manager_id;
}

    /**
     * Determine whether the user can update the model.
     */
    public function update(User $user, Task $task): bool
    {
       return $user->id === $task->executor_id || $user->id === $task->responsible_id;
    }

    /**
     * Determine whether the user can delete the model.
     */
    public function delete(User $user, Task $task): bool
    {
        //
    }

    /**
     * Determine whether the user can restore the model.
     */
    public function restore(User $user, Task $task): bool
    {
        //
    }

    /**
     * Determine whether the user can permanently delete the model.
     */
    public function forceDelete(User $user, Task $task): bool
    {
        //
    }
}
